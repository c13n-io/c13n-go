...
import (
	...
	model "github.com/c13n-io/c13n-backend/model"

	store "github.com/c13n-io/c13n-backend/store"
)
...
func (_m *Database) GetMessages(discussionUID uint64, pageOpts model.PageOptions) ([]store.MessageAggregate, error) {
	ret := _m.Called(discussionUID, pageOpts)

	var r0 []store.MessageAggregate
	if rf, ok := ret.Get(0).(func(uint64, model.PageOptions) []store.MessageAggregate); ok {
		r0 = rf(discussionUID, pageOpts)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]store.MessageAggregate)
		}
	}
